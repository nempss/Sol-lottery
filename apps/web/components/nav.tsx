...
const LINKS = [
  ...
  ...(LOTTERY_ENABLED ? [{ href: "/lottery", label: "Rewards" }] : []),
    ...
    <header className="sticky top-0 z-30 border-b border-line/80 bg-ink/80 backdrop-blur">
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-4">
        <Link href="/" className="flex items-center gap-2 font-medium tracking-tight">
          <img src={BRAND_LOGO} alt="" className="h-8 w-8 rounded-lg bg-white object-contain" />
          <span className="hidden sm:inline">{BRAND_NAME}</span>
        </Link>
        <nav className="hidden items-center gap-6 text-sm md:flex">
          {LINKS.map((l) => (
            <Link key={l.href} href={l.href} className={path === l.href ? "text-white" : "link-quiet"}>{l.label}</Link>
          ))}
        </nav>
        <WalletButton />
      </div>
